(ns gepredix.main
  "Kotodama WASM entrypoint for the Gepredix clean-room actor (L4) — Clojure port.
  ...
  facts (`gepredix.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gepredix")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"gepredix.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gepredix-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gepredix-compat/ready)
